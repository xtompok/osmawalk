WITH sq AS (
        ...
        FROM nodes
) UPDATE stops SET square1 = FLOOR((lat-sq.minlat)/100)*lonparts+FLOOR((lon-sq.minlon)/100) FROM sq;
...
WITH sq AS (
        SELECT  MIN(lon) AS minlon, MAX(lon) AS maxlon,
                ...
                CEIL((MAX(lat)-MIN(lat))/100) AS latparts
        FROM nodes
) UPDATE stops SET square2 = FLOOR((lat-sq.minlat+50)/100)*lonparts+FLOOR((lon-sq.minlon+50)/100) FROM sq;
